package minimumFuelCostToReportToTheCapital_2477;
...
public class Solution {
    ...
    public long minimumFuelCost(int[][] roads, int seats) {
        ...
        solve(graph, seats, 0, 0);

        return fuel;
    }

    private int solve(List<List<Integer>> graph, int seats, int prev, int cur) {
        int people = 1;
        int incoming = 0;
        for (int city: graph.get(cur)) {
            if (city != prev) {
                incoming = solve(graph, seats, cur, city);
